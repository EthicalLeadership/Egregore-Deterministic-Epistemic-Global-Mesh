...
class FactoryTab(ttk.Frame):
    ...
    def _poll(self, first: bool = False) -> None:
        ...
        for path in self._telemetry_files():
            ...
            if size < offset:
                # rotated/truncated: restart from the top
                offset = 0
                self._offsets[path] = 0
                self._buf[path] = ""
            if size == offset:
            ...
            chunk = self._buf.pop(path, "") + chunk
            lines = chunk.splitlines()
            # The last fragment is probably a partially written line; hold it
            # back until the next poll appends the rest of the line.
            if lines and not chunk.endswith("\n"):
                self._buf[path] = lines.pop()
            for line in lines:
                line = line.strip()
                if not line:
                    continue
                try:
                    ...
                    continue  # ignored: unparsable buffer line
