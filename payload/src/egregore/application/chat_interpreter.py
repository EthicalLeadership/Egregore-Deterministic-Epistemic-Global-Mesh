...
def _legacy_task_intent_enabled() -> bool:
    """Opt-in compatibility shim for raw `/agent` instructions.

    TODO(legacy-task-intent): remove after v0.8.0 or 2026-10-01, whichever lands first.
    """
    value = os.environ.get("EGREGORE_AGENT_LEGACY_TASK_INTENT", "0").strip().lower()
    return value in {"1", "true", "yes", "on"}


def _caller_identity_payload(context: ChatContext) -> dict[str, Any]:
...
def _caller_identity_token(context: ChatContext) -> str:
    token = context.env.get("caller_identity_token")
...
def _cmd_agent(args: list[str], context: ChatContext) -> dict[str, Any]:
    ...
    if not args:
        return {
            ...
            "command": "agent",
            "ok": False,
            "summary": "Usage: `/agent <name> <TaskIntent JSON or legacy instruction>`",
            "detail": None,
        }

    agent_name = args[0]
    raw_instruction = " ".join(args[1:]) if len(args) > 1 else ""
    ...
    agent_context = {
        ...
        "caller_identity": _caller_identity_payload(context),
        "caller_identity_token": _caller_identity_token(context),
        "identity_verifier": context.env.get("identity_verifier"),
        "allow_legacy_task_intent": _legacy_task_intent_enabled(),
    }
    result = runner.run(spec, raw_instruction, agent_context)
...
def _cmd_legal(args: list[str], context: ChatContext) -> dict[str, Any]:  # noqa: C901
    ...
    try:
        if _is_gguf_model(active_model):
            ...
            gguf_result = orchestrator.chat(messages, model_id=active_model)
            if gguf_result.ok:
                synthesis = gguf_result.text
            ...
            response = inference_service.execute(request)
            if response.message:
...
def _execute_inference_request(inference_service: Any, request: ChatRequest) -> ChatResponse:
    """Typed helper for multi-backend inference execution."""
    response = inference_service.execute(request)
    if not isinstance(response, ChatResponse):
        raise TypeError("Inference service returned a non-ChatResponse result")
    return response
