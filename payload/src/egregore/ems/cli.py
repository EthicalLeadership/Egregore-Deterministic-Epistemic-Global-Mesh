...
def cmd_promote(registry: EmsRegistry, args: argparse.Namespace) -> int:
    try:
        ...
        print(f"Promotion blocked for {args.model_id}: {exc}", file=sys.stderr)
        return 1
    print(
        f"Promotion approved for {approval['model_id']} by {approval['approved_by']} "
        f"at {approval['approved_at_ns']}"
    )
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    # promote
    p_promote = sub.add_parser("promote", help="Verify and record model promotion approval")
    p_promote.add_argument("model_id")
    p_promote.add_argument("checkpoints_dir")
    p_promote.add_argument("--approval-artifact", required=True)
    p_promote.add_argument("--approval-signature", required=True)
    p_promote.add_argument("--approval-public-key", required=True)
