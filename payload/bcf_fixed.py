...
class E(threading.Thread):
    def __init__(self, i, b, p):
        ...
        self.i = i; self.b = b; self.p = p
        self.running = True; self.k = True; self.tf = 0; self.tc = 0
    def run(self):
        while self.running:
...
class G(E):
    def __init__(self, i, b, p):
        super().__init__(i, b, p)
        self.d = None; self.g = False; self._ig()
    def _ig(self):
        try:
            import torch
            if torch.cuda.is_available():
                self.d = torch.device("cuda:0"); self.g = True
                print(f"  [G-{self.i}] CUDA")
            else:
                self.d = torch.device("cpu"); print(f"  [G-{self.i}] CPU")
        except:
            self.d = None; print(f"  [G-{self.i}] NO TORCH")
    def run(self):
        while self.running:
            w = self.b.popleft() if len(self.b) > 0 else None
            if w is None:
            ...
            if self.g and self.d:
                import torch
                a = torch.from_numpy(np.random.rand(1024, 1024).astype(np.float32)).float().to(self.d)
                b = torch.randn(1024, 1024, device=self.d, dtype=torch.float32)
                r = torch.matmul(a, b); torch.cuda.synchronize()
            else:
                a = np.random.rand(1024, 1024).astype(np.float32)
                b = np.random.rand(1024, 1024).astype(np.float32)
                np.dot(a, b); time.sleep(0.003)
...
class B:
    def __init__(self, s): self.s = s; self.b = deque(maxlen=s)
    def push(self, w): self.b.append(w)
    def popleft(self):
        if len(self.b) == 0: return None
...
def main():
    ...
    b = B(2048)
    cells = [G(i, b, p) if g else E(i, b, p) for i in range(16)]
    ...
    while time.time() - s < T:
        ...
        for _ in range(random.randint(2, 10)):
            ...
            if a: b.push((dt, dtc, dtg, tu, scl, ml, vr))
