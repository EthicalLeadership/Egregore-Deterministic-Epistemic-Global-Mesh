...
# Create the middleware file (same as before)
...
# Modify bootstrap.py to register the middleware using AST to find app assignment
...
# Find the assignment to app (e.g., app = FastAPI(...) or app = create_app(...))
app_assignment = None
for node in ast.walk(tree):
    if isinstance(node, ast.Assign):
        # Check if any target is named 'app'
...
if app_assignment is None:
    print("ERROR: could not locate assignment to 'app' in bootstrap.py")
    sys.exit(1)

# Get line number and indentation of the assignment
assign_line = app_assignment.lineno
# Indentation: count leading whitespace from the original text
lines = text.splitlines()
indent = len(lines[assign_line-1]) - len(lines[assign_line-1].lstrip())
indent_str = lines[assign_line-1][:indent]
...
if "tool_call_middleware" not in text:
    ...
    # Insert at top, preserving any shebang or encoding comments? We'll just add after first line if shebang
    if text.startswith("#!"):
        first_newline = text.index("\n")
        text = text[:first_newline+1] + import_line + text[first_newline+1:]
    else:
        text = import_line + text
    lines = text.splitlines()

# Add middleware registration after app assignment line
reg_line = f"{indent_str}app.add_middleware(ToolCallMiddleware)\n"
# Find the index in lines list (1-based)
insert_idx = assign_line  # after the assignment line (1-based)
# But we need to adjust for possible added import line above
if "tool_call_middleware" in text and "from egregore.http_api.http.middleware.tool_call_middleware" in text:
    # The import was inserted at top, so line numbers shifted by 1
    insert_idx += 1

lines = text.splitlines()
# Insert after the assignment line (0-based index = insert_idx)
lines.insert(insert_idx, reg_line.rstrip('\n'))
text = "\n".join(lines) + "\n"
