// Fixed header: branding, status LEDs, tier badge, UTC clock.
...
export function Header({ fps, reducedMotion, onToggleMotion }: {
  fps: number;
  ...
  return (
    <header className="flex h-11 shrink-0 items-center gap-4 border-b border-[#1a2540] bg-[#0c1220] px-4">
      ...
      <div className="hidden items-center gap-4 md:flex">
        <Led label="Renderer" ok={fps > 30} warn={fps <= 30} />
        <Led label="Store" ok />
        <Led label="Coordination 200ms" ok />
      </div>

      <div className="ml-auto flex items-center gap-4">
        <span className="hidden font-mono text-[9px] text-slate-500 sm:block">
          {fps} FPS · CANVAS2D DEMO <span className="text-slate-600">(prod: GPU-first)</span>
        </span>
        <button
