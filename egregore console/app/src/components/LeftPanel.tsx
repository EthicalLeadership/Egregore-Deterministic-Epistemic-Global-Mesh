// Left rail: layer controls, metro watchlist, scale-tier statechart with the
// live hysteresis band meter, and the band tuner (labeled as heuristic).
...
export function LeftPanel({ cameraK }: { cameraK: number }) {
  const state = useStore();

  // band meter: position of k between the current tier edges on a log scale
    ...
    <div className="flex h-full flex-col overflow-y-auto">
      ...
      <div className="border-b border-[#1a2540] px-3 py-2.5">
        ...
        <div className="mt-1 border border-[#1a2540] p-2">
          ...
          <div className="mt-1 text-[8px] leading-tight text-slate-600">
            Tunable heuristic — not an empirical constant. Prevents tier flicker
            near boundaries.
          </div>
        </div>
      </div>
      ...
      <div className="px-3 py-2.5">
        ...
        {CITIES.slice(0, 8).map((c) => (
          ...
          >
            <span className={`text-[10.5px] ${c.hq ? 'text-cyan-300' : 'text-slate-300 group-hover:text-white'}`}>
              {c.name}
              {c.hq && <span className="ml-1.5 border border-cyan-400/50 px-1 text-[8px] text-cyan-400">HQ</span>}
            </span>
            <span className="font-mono text-[9px] text-slate-600 group-hover:text-cyan-400">
              {c.lon.toFixed(1)},{c.lat.toFixed(1)} →
            </span>
          </button>
        ))}
      </div>
