...
def _get_rbac_authz_provider():

    return RBACAuthzProvider(list(USERS.values()))


_DEFAULT_EXECUTOR = CorePlaneGenerateDossierExecutor(
    authz=_get_rbac_authz_provider(),
...
def _ensure_case_seeded_in_store(
    *, store: Any, organization_id: str, case_id: str
    ...
    The HTTP layer must not depend on internal store details; this module owns
    seeding semantics and only requires stores that expose:
    - get_case_state(...)
...
def _get_model_catalog() -> object | None:
    ...
    # Lazy import to keep application module import fast and deterministic.
...
def generate_v1_dossier(
    *, request: DossierGenerateRequest, vertical: str | None = None
    ...
    Ownership:
    - seeding internal stores
    - selecting default vs vertical compute policy
    - delegating to DossierGenerateService

    The HTTP interface layer must remain thin and should not import or
    construct core executors or journaling providers directly.
    """
