...
def create_app() -> FastAPI:  # noqa: C901
    ...
    async def anchorum_case_index_stats(case_id: str) -> Any:
        ...
        # index_stats() opens a ChromaDB PersistentClient. On a large store
        # that blocks for a long time; running it inside the threadpool keeps
        # the event loop responsive so other requests are not frozen behind it.
        return await run_in_threadpool(case_rag.index_stats, case_id)
    ...
    async def anchorum_case_index_health(case_id: str) -> Any:
        ...
        try:
            _ = await run_in_threadpool(
                case_rag.query_case, case_id, "health check", 1
            )
